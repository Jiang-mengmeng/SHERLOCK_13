...
volatile int synchro; // acceder a la vrai adresse, vrai valeur

int gOver = 0; // 0: jeu en cours, 1: victoire, -1: défaite
...
void *fn_serveur_tcp(void *arg)
{
	...
	while (1)
	{
		...
		synchro = 1; // Message prêt à être traité  
		pthread_mutex_unlock(&mutex); 

		while (synchro); // block l'accepter le nouveau demande
    }
}
...
int main(int argc, char ** argv)
{
    ...
    while (!quit)
    {
		if (SDL_PollEvent(&event)) // detecter
		{
			//printf("un event\n");
			switch (event.type)
			{
        ...
        if (synchro == 1)
        {
			...
			switch (gbuffer[0])
			{
				...
				case 'L':
					...
					// printf("%s %s %s %s", gNames[0], gNames[1], gNames[2], gNames[3]);
					break;
				// Message 'D' : le joueur recoit ses trois cartes
				case 'D':
					// RAJOUTER DU CODE ICI
					sscanf(gbuffer, "%c %d %d %d %d %d %d %d %d %d %d %d", &com, b, b+1, b+2, tableCartes[gId], tableCartes[gId]+1, tableCartes[gId]+2, tableCartes[gId]+3, tableCartes[gId]+4, tableCartes[gId]+5, tableCartes[gId]+6, tableCartes[gId]+7);
					break;
				// Message 'M' : le joueur recoit le n° du joueur courant
				// Cela permet d'affecter goEnabled pour autoriser l'affichage du bouton go
				case 'M':
					// RAJOUTER DU CODE ICI
					sscanf(gbuffer, "%c %d", &com, &id);
				...
				case 'V':
					// RAJOUTER DU CODE ICI
					int val;
					sscanf(gbuffer, "%c %d %d", &com, &id, &val);
					tableCartes[id][objetSel] = val;
					break;
				// Message 'W' : le joueur recoit le n° du joueur qui gagne
				case 'W':
					sscanf(gbuffer, "%c %d", &com, &id);
					if(id == gId) gOver = 1;
					else gOver = -1;
					break;
				// Message 'F' : le joueur recoit le n° du joueur qui perdu
				case 'F':
					sscanf(gbuffer, "%c %d", &com, &id);
					if(id == gId) gOver = -1;
					break;
		...
		for (i=0;i<4;i++)
			for (j=0;j<8;j++)
			{
				...
				{
					char mess[10];
					if (tableCartes[i][j]==100) // 100 represente le joueur a ce symbole
						sprintf(mess, "*");
					else
		...
		// La fin du jeu
		if (gOver != 0) {
			SDL_Color color = {255, 0, 0}; // Rouge pour la défaite, Vert pour la victoire
			char message[256];
			if (gOver == 1) {
				sprintf(message, "VICTOIRE ! Vous avez gagne !");
				color = (SDL_Color){0, 255, 0};
			} else {
				sprintf(message, "DEFAITE ! Vous avez perdu !");
			}
			
			SDL_Surface* surfaceMessage = TTF_RenderText_Solid(Sans, message, color);
			SDL_Texture* Message = SDL_CreateTextureFromSurface(renderer, surfaceMessage);
			
			SDL_Rect Message_rect = {400, 550, surfaceMessage->w, surfaceMessage->h};
			SDL_RenderCopy(renderer, Message, NULL, &Message_rect);
