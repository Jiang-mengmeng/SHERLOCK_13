...
int main(int argc, char *argv[])
{
	...
	nbClientsF = 0;
	for(i=0; i<3; i++) ClientsF[i] = -1;

	while (1) {
		if(fsmServer != 2){ // Ne traite que si le jeu n'est pas terminé
			...
			if (fsmServer == 0) {
				...
				switch (buffer[0]) {
					case 'G':
						// RAJOUTER DU CODE ICI
						// int guilt;
						sscanf(buffer, "%c %d %d", &com, &joueurCourant, &id);
						if(id == deck[12]){
							sprintf(reply, "W %d", joueurCourant);
							fsmServer = 2; // État "jeu terminé"
							joueurCourant = -1;
						} else {
							sprintf(reply, "F %d", joueurCourant);
							ClientsF[nbClientsF] = joueurCourant;
							nbClientsF++;
						}
						broadcastMessage(reply);
						if(nbClientsF == 3){
							for(i=0; i<4; i++){ // On cherche dans quatre joueurs qui gagne
								if(i!=ClientsF[0] && i!=ClientsF[1] && i!=ClientsF[2]) id = i;
							}
					...
					case 'O':
						...
						for(i=0; i<4; i++){
							if(i != joueurCourant){
								if(tableCartes[i][id] != 0){
									sprintf(reply, "V %d 100", i); // 100 represente le joueur a ce symbole
									sendMessageToClient(tcpClients[joueurCourant].ipAddress, tcpClients[joueurCourant].port, reply);
								} else {
									sprintf(reply, "V %d 0", i);
									sendMessageToClient(tcpClients[joueurCourant].ipAddress, tcpClients[joueurCourant].port, reply);
								}
							}
						}
					...
					case 'S':
						// RAJOUTER DU CODE ICI
						int joueurSel;
						sscanf(buffer, "%c %d %d %d", &com, &joueurCourant, &joueurSel, &id);
						sprintf(reply, "V %d %d", joueurSel, tableCartes[joueurSel][id]);
						sendMessageToClient(tcpClients[joueurCourant].ipAddress, tcpClients[joueurCourant].port, reply);
						break;
					default:
						break;
			...
			if(joueurCourant != -1){
				...
				for(i=0; i<3; i++){
					if(joueurCourant == ClientsF[i]) joueurCourant++; // On saut les joueurs perdus
					if(joueurCourant == 4) joueurCourant = 0;
				}
			}
			sprintf(reply, "M %d", joueurCourant);
